...
function setWeekdayGreeting() {
    ...
    document.getElementById('welcome-title').innerText = `Junkyard Digs Hub — Happy ${currentDay}!`;
}
...
function initAiChat() {
    ...
    messagesContainer.innerHTML = `
        <div class="message bot">
            Welcome to the <strong>Junkyard Digs Command Channel</strong>! Happy ${currentDay}! 🎮<br><br>
            Would you like to examine our latest videos, receive a personalized recommendation, or sign up for the Junkyard Digs Creators' Club membership tier?
        </div>
    `;
}
...
function sendAiMessage() {
    ...
    setTimeout(() => {
        ...
        if (normalized.includes('latest') || normalized.includes('video') || normalized.includes('current')) {
            botMsg.innerHTML = "Pulling data matrix... Junkyard Digs's current active broadcast file is situated at <code>https://github.iointro.mp4</code>.";
        } else if (normalized.includes('club') || normalized.includes('member') || normalized.includes('join')) {
            botMsg.innerHTML = "You can instantly process your Junkyard Digs Creators' Club membership activation application by engaging the interactive submission button inside the dashed dashboard panel on this page!";
        } else {
            botMsg.innerHTML = "I am specifically mapped to monitor the localized <code>/jd/</code> directory structures. Ask me about our background files or timeline stream elements!";
        }
